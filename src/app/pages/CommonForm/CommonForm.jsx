...
const CommonForm = () => {
  ...
  const normalizeFormData = (data, config) => {
    ...
    if (
      config.isDescriptionListRequired &&
      merged.descriptionList.length === 0
    ) {
  ...
  const buildPayload = (data, config) => {
    ...
    // ----------------------------
    // ✅ SINGLE IMAGE -> payload.image = base64 (ONLY when uploaded)
    // ----------------------------
    let singleImageBase64 = "";
    if (config.isSingleImageRequired) {
      // prefer singleImage, fallback to first multipleImage if you want
      const src = data.singleImage || data.multipleImage?.[0] || "";
      singleImageBase64 = isDataUrl(src) ? stripDataUrlPrefix(src) : "";
    }

    // ----------------------------
    // ✅ MULTIPLE IMAGES -> payload.imageList = [{imageId, url(base64)}]
    // ----------------------------
    let imageList = [];
    if (config.isMultipleImageRequired) {
      const multiSource =
        Array.isArray(data.multipleImage) && data.multipleImage.length > 0
          ? data.multipleImage
          : data.singleImage
          ? [data.singleImage]
          : [];

      imageList = multiSource.map((img, index) => {
        const existingId =
          data.blogImageIds?.[index] || data.blogImageId || null;

        // If user uploaded a new image => send base64
        if (isDataUrl(img)) {
          return {
            imageId: existingId, // can be null for add
            url: stripDataUrlPrefix(img), // ✅ base64 string only
          };
        }

        // If it's already an existing URL (edit case) and not changed:
        // keep imageId and send empty string as url
        return {
          imageId: existingId,
          url: "",
        };
    ...
    const payload = {
      ...
      image: config.isSingleImageRequired ? singleImageBase64 : "",
      imageList: config.isMultipleImageRequired ? imageList : [],

      descriptionList,
      date: data.date ? dayjs(data.date).toISOString() : null,
      urlLink: attachmentMode === "url" ? data.attachmentUrl || "" : "",
      uploadDocument:
        attachmentMode === "file" ? data.attachmentBase64 || "" : "",
      year: toNumber(data.year),
      month: data.month || "",
      fromDate: data.dateRange?.[0]
        ? dayjs(data.dateRange[0]).toISOString()
        : null,
      toDate: data.dateRange?.[1]
        ? dayjs(data.dateRange[1]).toISOString()
        : null,
      minAge: toNumber(data.minAge),
      descriptionListHeader: data.descriptionList?.[0]?.header || "",
    };

    if (data.blogId) payload.blogId = data.blogId;
  ...
  const mapApiItemToForm = (item, config) => {
    const imageUrls = (item.imageList || [])
    ...
    const mapped = {
      ...
      singleImage:
        imageUrls.length <= 1 ? imageUrls[0] || fallbackImage : fallbackImage,
  ...
  const handleDescriptionListChange = (index, field, value) => {
    ...
    setErrors((prev) => {
      if (!prev.descriptionListItems || !prev.descriptionListItems[index])
        return prev;
      const updated = { ...prev.descriptionListItems };
  ...
  const handleDelete = async (index) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      ...
      setIsDeleting(true);
      const { statusCode, message, error } = await deleteApi(DELETE_DATA, {
        params: { blogImageId },
      });
  ...
  const validateForm = () => {
    ...
    if (
      ...
      (!formData.multipleImage || formData.multipleImage.length === 0)
    ) {
      ...
      <div className="item-view-content">
            ...
            <div className="view-row gallery-row">
              {item.multipleImage.map((img, idx) => (
                <img
                  ...
                  src={img}
                  alt="Preview"
                  className="view-image-preview small"
                />
              ))}
            </div>
          ) : (
        ...
        <div className="view-fields">
          ...
          {currentConfig.isSubTitleRequired && (
            <div className="view-field">
              <span className="view-label">
                {currentConfig.subTitleName || "Sub Title"}
              </span>
          ...
          {currentConfig.isDescriptionRequired && (
            <div className="view-field">
              <span className="view-label">
                {currentConfig.descriptionName || "Description"}
              </span>
                    ...
                    <div key={idx} className="view-list-item">
                      ...
                      {currentConfig.isDescriptionListHeaderRequired && (
                        <div className="view-subfield">
                          <span className="view-sub-label">
                            {currentConfig.descriptionListHeaderName ||
                              "Header"}
                            :
                          </span>
                          <span className="view-sub-value">
                            {formatValue(listItem.header)}
                          </span>
                        </div>
                      )}
                      {currentConfig.isDescriptionListTitleRequired && (
                        <div className="view-subfield">
                          <span className="view-sub-label">
                            {currentConfig.descriptionListTitleName || "Title"}:
                          </span>
                          <span className="view-sub-value">
                            {formatValue(listItem.title)}
                          </span>
                        </div>
                      )}
                      {currentConfig.isDescriptionListContentRequired && (
                        <div className="view-subfield">
                          <span className="view-sub-label">
                            {currentConfig.descriptionListContentName ||
                              "Content"}
                            :
                          </span>
                          <span className="view-sub-value">
                            {formatValue(listItem.content)}
                          </span>
                        </div>
                      )}
                    </div>
          ...
          {(currentConfig.isDateRequired ||
            ...
            <div className="view-meta">
              ...
              {currentConfig.isDateRangeRequired && (
                <div className="view-field">
                  <span className="view-label">
                    {currentConfig.dateRangeName || "Date Range"}
                  </span>
                  <span className="view-value">
                    {formatDateRangeValue(item.dateRange)}
                  </span>
                </div>
              )}
              {currentConfig.isYearRequired && (
              ...
              {currentConfig.isMinAgeRequired && (
                <div className="view-field">
                  <span className="view-label">
                    {currentConfig.minAgeName || "Min Age"}
                  </span>
              ...
              {currentConfig.isAttachmentRequired && (
                <div className="view-field">
                  <span className="view-label">
                    {currentConfig.attachementName || "Attachment"}
                  </span>
                  <span className="view-value">
                    {formatValue(
                      item.attachmentUrl ||
  ...
  return (
    <div className="common-form-page">
      ...
      <CustomModal
        open={isModalOpen}
        title={`${editingItemIndex !== null ? "Edit" : "Add"} ${
          currentConfig.typeName
        }`}
        ...
        <form className="common-form-modal-content">
            ...
            <div className="form-section">
              {currentConfig.isSingleImageRequired && (
                <ImageUploadField
                  ...
                  aspectRatio={(() => {
                    ...
                    const [w, h] = currentConfig.singleImageCropRatio
                      .split(":")
                      .map(Number);
                    return w / h || 1;
                  })()}
              ...
              {currentConfig.isMultipleImageRequired && (
                <MultiImageUploadField
                  title={
                    currentConfig.multipleImageUploadName || "Multiple Image"
                  }
                  name="multipleImage"
          ...
          {currentConfig.isDescriptionListRequired && (
            <div className="form-section">
              <div className="section-header-row">
                <h3>List Items</h3>
                <ButtonComponent
                  onClick={addDescriptionListItem}
                  size="small"
                  variant="secondary"
                  type="button"
                >
                ...
                <div key={index} className="description-list-item">
                  <div className="item-header">
                    ...
                    <button
                      ...
                      onClick={() => removeDescriptionListItem(index)}
                    >
                      Remove
                    </button>
                  </div>
                  <div className="form-grid-modal">
                    {currentConfig.isDescriptionListHeaderRequired && (
                      <InputField
                        title={
                          currentConfig.descriptionListHeaderName || "Header"
                        }
                        value={item.header}
                        onChange={(e) =>
                    ...
                    {currentConfig.isDescriptionListContentRequired && (
                      <TextAreaField
                        title={
                          currentConfig.descriptionListContentName || "Content"
                        }
                        value={item.content}
                        onChange={(e) =>
                          handleDescriptionListChange(
                        ...
                        errorText={
                          errors.descriptionListItems?.[index]?.content
                        }
                        rows={2}
                      />
                    )}
          ...
          {(currentConfig.isDateRequired ||
            ...
            <div className="form-section">
              ...
              {currentConfig.isMinAgeRequired && (
                <InputField
                  title={currentConfig.minAgeName}
                  name="minAge"
                  type="number"
              ...
              {currentConfig.isDateRangeRequired && (
                <div className="forminput">
                  ...
                  />
                  {errors.dateRange && (
                    <span className="input-error-text">{errors.dateRange}</span>
                  )}
                </div>
              )}
              {currentConfig.isAttachmentRequired && (
                <div className="attachment-block">
                  ...
                  {attachmentMode === "file" ? (
                    ...
                    <InputField
                      ...
                      onChange={handleChange}
                      placeholder="URL"
                      errorText={errors.attachmentUrl}
                    />
                  )}
                </div>
              )}
